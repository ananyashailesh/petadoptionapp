...
class GoogleDriveService {
  ...
  static const _credentialsPath = 'assets/service_account.json';
  static const _folderId = '1k1zlvfoKQ9oldLOvc4P4UueFlrvJOsOD';
      ...
      try {
        ...
        if (file is List<int>) {
          ...
          uploadResponse = await driveApi.files.create(
            fileMetadata,
            uploadMedia: drive.Media(Stream.fromIterable([file]), file.length),
          ...
          uploadResponse = await driveApi.files.create(
            fileMetadata,
            uploadMedia: drive.Media(ioFile.openRead(), await ioFile.length()),
            $fields: 'id,webViewLink',
          );
        }
        ...
        final sharedFile =
            await driveApi.files.get(uploadResponse.id!, $fields: 'webViewLink')
                as drive.File;
